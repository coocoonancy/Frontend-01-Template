function enabeleGesture(el) {
    ...
    let start = (p, ctxs) => {
        ...
        ctxs.timer = setTimeout(() => {
            ...
            el.dispatchEvent(new CustomEvent('pressstart', {}));
        }, 500)
        // console.log('start', p.clientX, p.clientY, dx, dy);
    }
    let move = (p, ctxs) => {
        let dx = p.clientX - ctxs.startX, dy = p.clientY - ctxs.startY;
        if (ctxs.isPress) {
            el.dispatchEvent(new CustomEvent('presscancel', {}));
        }
        ...
        // console.log('move', p.clientX, p.clientY, dx, dy);
    }
    let end = (p, ctxs) => {
        if (ctxs.isPan) {
            let dx = p.clientX - ctxs.startX, dy = p.clientY - ctxs.startY;
        ...
        if (ctxs.isPress) {
            el.dispatchEvent(new CustomEvent('pressend', {}));
            console.log('pressend');
        }
        clearTimeout(ctxs.timer);
        // console.log('end', p.clientX, p.clientY, dx, dy);
    }
    let cancel = (p, ctxs) => {
        clearTimeout(ctxs.timer);
        // console.log('cancel', p.clientX, p.clientY, dx, dy);
    }
}
